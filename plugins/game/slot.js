...
module.exports = {
  ...
  tags: ["game"],
  help: ["slot"],
  run: async (bot, { msg,command }) => {
    
    global.slots = global.slots ? global.slots : {}
        if (msg.sender in global.slots) return msg.sendReply('Masih ada Yang Melakukan Slots Disini, Tunggu Sampai selesai!!')
        else global.slots[msg.sender] = true
        try { 
            if (msg.args.length < 1) return msg.sendReply(`Gunakan format *${command} [jumlah]*
    contoh *${command} 10*`)
            let count = msg.args[0]
    ...
    ${pickRandom(['🍊', '🍇', '🍉', '🍌', '🍍'])}|${pickRandom(['🍊', '🍇', '🍉', '🍌', '🍍'])}|${pickRandom(['🍊', '🍇', '🍉', '🍌', '🍍'])}
            ...
            if (spin1 == spin2 && spin2 == spin3 && spin3 == spin4 && spin4 == spin5 && spin5 == spin6 && spin6 == spin7 && spin7 == spin8 && spin8 == spin9) {
                ...
                user.limit += Math.ceil(count * 4)
            } else if (spin4 == spin5 && spin5  == spin6) {
               WinOrLose = 'JACKPOT🥳'
               Hadiah = `+${Math.ceil(count * 2)}`
               user.limit += Math.ceil(count * 2)
    ...
    *${WinOrLose}* *${Hadiah}*
    `)
            ...
            delete global.slots[msg.sender]
        }
